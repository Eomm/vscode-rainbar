...
module.exports = async function applyColor ({ state, force }) {
  const {
    hue,
    applyTo,
    overwrite
  ...
  const palette = await generatePalette(hue, lastPaletteTitleApplied)
  ...
  try {
    await state.write({
      lastPaletteTitleApplied: palette.title
    })
  } catch (error) {
